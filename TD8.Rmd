...
avec la puissance exprimée en kiloWatt $kW$, $\rho$ la densité de l'air égale à 1.225 $kg.m^{-3}$,
...
Toutes les puissances sont données en $kW$.
...
dta %>% ggplot() + aes (x = lnV, y = lnP) + geom_point() + geom_smooth(method = "lm")
...
dta %>% ggplot() + aes (x = lnV, y = lnP, col = SecMat) + geom_point() + geom_smooth(method = "lm")
```
