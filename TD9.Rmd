...
###  Prédiction du volume de bois exploitable 
   ...
   Pour évaluer le volume de bois exploitable d'un arbre, il est fréquent d'assimiler le tronc à un cylindre. Dans ce cas, on mesure le diamètre à hauteur de poitrine, noté DBH (Diameter at Breast Height) et en notant *H* la hauteur du tron,  on évalue le volume *V* par la formule :
   $$V = D^2 H \frac{\pi}{4}. $$
   
* En passant au logarithme, écrire une relation liant le logarithme du diamètre diamètre au logarithme de de *V/H*.

Des données recueillies sur des pins Douglas sont diponibles dan sle jeu de données [douglas](douglas.csv)
...
* Mettre en place un test statistique permettant de conclure qu'il existe un lien entre le logarithme  du diamètre et le logarithme du ration Volume sur Hauteur.

* Si le tronc d'arbre est véritablement un cylindre, qu'elle serait la valeur du paramètre mesurant l'effet du log du diamètre sur le ratioVH ?
