...
* Ecrire un modèle permettant de mesurer un effet potentiel de la compote sur la note de saveur de pomme crue.  

* Indiquer les hypothèses testées, la stitistique de test et sa loi sous $H_0$, puis conclure sur l'existence d'une différence entre les compotes sur la saveur de pomme crue.

```{r modele1, echo = FALSE, results='markup'}
mod1.compote <- lm(S.pom.crue ~ compote,data=dta)
mod0 <- lm(S.pom.crue~1,data=dta)
anova(mod0,mod1.compote)
```

### Prise en compte de la variabilité associée aux juges.

Les premiers graphiques semblaient mettre en évidence un effet de la copote, le résultat obtenu est donc assez surprenant. Quelles peuvent en être les causes ?
...
```{r testGlobal, echo = FALSE, results='markup'}
mod2 <- lm(S.pom.crue ~ compote + juge,data=dta)
mod1.compote <- lm(S.pom.crue ~ compote,data=dta)
mod1.juge <- lm(S.pom.crue ~ juge,data=dta)
anova(mod0, mod2)
anova(mod1.juge, mod2)
anova(mod1.compote, mod2)
```

* Dans le modèle utilisé dans la première partie,  où se trouve la variabilité associée aux juges ? Pourquoi cela est-il gênant lorsqu'on s'intéresse à tester un effet compote ?
...
On souhaite maintenant identifier quelles compotes ont des saveurs de pomme crue différente. On souhaite pour ceci reproduire l'approche mise en oeuvre au TD précédant pour comparer les carures des sportifs à l'aide de la fonction`emmeans`.

```{r emmeans, echo = FALSE, results='markup', fig.show='asis'}
library(emmeans)
