...
On cherche à évaluer comment sont perçues 6 compotes de pommes au niveau de la saveur de pomme crue. Une dégustation est mise en place et 6 juges vont déguster chacune des 6 copmotes. Les notes sont attribuées sur une échelle allant de 0 à 10.
...
```{r import_data}
dta <- read.table('C:/Users/husson/AOBox/TD_2021_partage/compote.csv', sep = ';' , stringsAsFactors = TRUE, header = TRUE)
...
* Visualisez les données pour avoir une idée des saveurs de pomme crue selon les compotes et calculez quelques statistiques descriptives par compote.
...
* Dire à quelle question répond chacune des tests ci-dessous.
* Par rapport à la problématique permettant de savoir s'il y a des différences de saveur de pomme crue entre les compotes, dire quels tests vous allez commenter.
...
* La fonction `Anova` (avec un A majuscule) du package `car` (qu'il vous faudra sûrement installer) permet de retrouver en une seule commande les probabilités critiques de chacun des effets. Retrouver ces probabilités critiques.
...
```{r emmeans, echo = TRUE, results='markup'}
library(emmeans)
...
On peut aussi tester si la saveur de pomme crue d'une compote particulière est égale ou différente de la moyenne de l'ensemble des compotes. La moyenne correspond à des coefficient de -1.6 pour chaque compote, et donc si on compare la chaque compote à la moyenne, on écria:
