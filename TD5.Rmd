...
Une fois toutes les données recueillies, considérons deux groupes : les données évaluées par les garçons et
les données évaluées par les filles.
...
* Saisir les données dans un fichier `angle.csv`.
...
```{r import_data}
#don_Angle <- read.table('http://dsr-rennes.github.io/data/angle.csv', sep = ';' , stringsAsFactors = TRUE, header = TRUE)
don_Angle <- read.table('angle.csv', sep = ';' , stringsAsFactors = TRUE, header = TRUE)
```

#### Visualisation des données

* Visualiser les données pour avoir une idée de la distribution des angles pour les garçons, et pour les filles et calculez quelques statistiques par sexe.
...
```{r testMoyH}
don_Angle  %>% filter(Sexe=="H") %>% select(Angle) %>% t.test(mu=angle_ref, alternative = "less")
```
