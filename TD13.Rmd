...
EN préparation d'une future expérimentation en champs,  on a comparé, dans deux types de sols, trois types de sondes destinées à prélever des échantillons de terre, en effectuant plusieurs prélèvement et en mesurant la teneur en anhydride phosphorique ($P_20_5$) en mg pour 100 grammes de terre sèche.  On s'intéresse principalement aux différences qui pourraient exister d'un type de sondes à l'autre, et aux interférences éventuelles des typpes de sondes avec les types de sols. Les données sont disponibles dans le fichier [sonde_sol.csv](sonde_sol.csv).


* Importer les données et les repséenter sous fomre d'un boxplot dans lequel l'abcisse est le sol, ce boxplot étant rempli par une couleur qui représente la sonde.
...
* Mettre en place un test staistique permettant de répondre à la question "Le taux d'ahnydrire phosphaté est-il le même dans tous les types de sol, pour tous les sortes de sonde ?" (Donner les hypothèses H$_0$ et H$_1$)
...
* Donner la loi de la statistique de test sous H$_0$ en justifiant le sdegrés de liberté.

Puisque les teneurs en anhydirdes phosphatés ne sont pas identique pour toutes les sondes sur tou les sols, on souhaite identifier si la variabilité provient du sol, des sondes ou des deux.

* Quel test pouvez vous proposer pour répondre à la question, les différences entre les sondes sont elles les mêmes quelque soit le type de sol ?
...
* La différence de teneur en anhydride phosphatée entre les sols ayant été prise en compte, peut on envore voir une différence de mesure entre les sondes ?
...
* Quelle est la teneur en anhydride phosphaté prédite par le modèle sans interaction lorsqu'il mesuré avec la sonde 2 sur le sol 2. Même question pour la sonde 2 et le sol 1, puis la sonde 1 et le sol 1.
...
* Comparer aux résultats obtenus avec la commande `Anova` du package `car` (prétez attention au A et non a)
