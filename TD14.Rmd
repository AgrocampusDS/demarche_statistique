...
* Visualisation de données
  ...
  - Visualiser un effet potenteil d'interaction entre deux facteurs sur une variable réponse
...
On souhaite maintenant tester un potentiel effet traitement. Pour ce faire, on commence pare tester l'existence d'une potentielle interaction.
...
On ajuste maintenant le modèle d'analyse de la variance à 2 facteurs sans inteactions.
...
Nous cherchons à comprendre si l'effet d'une naissance précoce ets le même pour les garçons et les filles.
...
summary(Mcomp)
predict(Mcomp, newdata = data.frame(Sexe = 'M', Nbsem = -2))
```
...
* Pour répondre à cette question, on propose d'utiliser le code suivant utilisant le package emmeans. Un poids moyen pour les garçcons et un poids pour les filles est calculé. A quel nombre de semaines correspond-il ? 
