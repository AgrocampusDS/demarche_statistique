...
```{r back_to_TD1}
penguins <- read.table('https://dsr-rennes.github.io/data/penguins.csv', sep = ';', header = TRUE)
...
Il est possible de sauvegarder ce jeu de données simplifier dans  l'objet penguins lui-même, c'est ce qu'illustre la ligne de commande suivante :
...
* Dans le jeu de données penguins, le sexe de certains individus n'est pas renseigné. On peut identifier ces individus grâce à la commande
...
Pour obtenir un histogramme par sexe, il faut modifier le comportement par défaut et indiquer que l'on veut deux histogrammes différents et non pas représenter dans chaque classe la proportion de mâles et de femelles. Il faut modifier l'argument `position` par défaut.
...
* Rappeler la nature des variables longueur du bec et longueur des nageoires. Quelle sorte de graphique semple le plus adpatée ?

* Utiliser la fonction graphique `geom_point` en précisant bien les abcisses et les ordonnées pour obtenir une visulaisation du lien entre  la longueur des nageaoires sur la longueur du bec.
...
manchots %>% ggplot() + 
  geom_point(aes(x = longueur_nageoire, y = longueur_bec, col = sexe)) +
  ggtitle('La longueur du bec en fonction de la longueur des nageoires selon le sexe')
```
...
Il est également possible de représenter les données elles-mêmes. On peut utiliser la commande `geom_point` qui va dessiner un point aux abcisses et ordonnées spécifiées.
...
Le résultat n'est pas très satisfaisant, car tous les points ayant la même abcisse, il est difficile de les différencier. Il est possible de perturber légèrement l'abcisse de manière à mieux distinguer les différentes mesures. 
...
- limiter la perturbation de l'abcisse,
