...
* Utiliser la fonction graphique `geom_point` en précisant bien les abscisses et les ordonnées pour obtenir une visualisation du lien entre  la longueur des nageoires sur la longueur du bec.
...
manchots %>% ggplot(aes(x = sexe, y = longueur_bec)) + 
  geom_boxplot()  +
...
```{r boxplot_labs_points_dotplot, echo = TRUE}
manchots %>% ggplot(aes(x = sexe, y = longueur_bec, fill = sexe)) +
  geom_boxplot()  +
...
```{r boxplot_labs_points_dotplot_nice_col, echo = TRUE}
manchots %>% ggplot(aes(x = sexe, y = longueur_bec, fill = sexe)) +
