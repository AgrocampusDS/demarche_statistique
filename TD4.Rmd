...
Reprenons la même problématique d'élevage bovin que dans le TD précédent. Cette fois-ci on souhaiterait réaliser une étude sur des Charolaises et des Limousines.

* Avec un effectif de 60 vaches de chaque race, quelle serait alors la différence de  gains moyens théoriques entre ces races de vache potentiellement détectable avec une probabilité de 0.90 ?
 ?

```{r delta}
power.t.test(sd = 90,  power = .9, n = 60, sig.level = .05)
```



* Combien de vaches suffirait-il d'avoir pour réussir à détecter un écart d'environ 100g entre les gains moyens théoriques des Charolaises et des Limousines avec une probabilité de 0.90 ?

```{r taille}
power.t.test(sd = 90,  delta = 100, power = .9, sig.level = .05)
```
