...
```{r import_data_V1}
data.sport.V1 <- read.table('/Users/memily/AOBox/Enseignement/2021-2022/DemarcheStat/TD_ME/dataSportV1.csv', sep = ';' , stringsAsFactors = TRUE, header = TRUE)
```
...
```{r import_data_V2}
data.sport.V2 <- read.table('/Users/memily/AOBox/Enseignement/2021-2022/DemarcheStat/TD_ME/dataSportV2.csv', sep = ';' , stringsAsFactors = TRUE, header = TRUE)
```
...
```{r testCoeff3_Vf, echo = FALSE, results='markup',eval=FALSE}
library(emmeans)
...
##### Test Post-Hoc (Pour aller plus loin)

Les scientifiques cherchent à présent à savoir si les sports collectifs donnent une carrure différente de celle pour les sportifs pratiquant un sport individuel.

*  Exprimer cette question sous la forme d'un test d'hypothèse qui s'appuie sur le modèle global.

* A l'aide de la fonction *glht* du package *multcomp*, effectuer ce test sous R.

```{r testCoeff4_Vf, echo = FALSE, results='markup', eval=FALSE}
require(multcomp)
#test_AllPairs <- glht(mod.Vf,linfct=mcp(Sport="Tukey"))
#summary(test_AllPairs, test = adjusted("bonferroni"))

test_IndividualVsCollectif <- glht(mod.Vf,linfct=mcp(Sport=c(1,-1,-1,1,1,-1)))
summary(test_IndividualVsCollectif, test = adjusted("bonferroni"))
```
