...
Un groupe industriel commercialisant du café souhaite comparer les cafés provenant de différents lieux de production à partir de leur profil de composition physico-chimique, dont une des composantes importantes est le taux de matière sèche (`DM`). Pour cela, il s'appuie sur des données contenant le lieu de production, codé par un entier allant de `1` à `7`, de 240 échantillons de café disponible sur la page 

### Description du jeu de données sur la qualité du  café

#### Importation du jeu de données

* Importer le jeu de données `cafe.csv` disponible sur la page des [jeux de données ](https://dsr-rennes.github.io/data.html) dans un  objet nommé `cafe`  en utilisant un script `TD3_script.R` que vous aurez créé dans votre répertoire de projet.

```{r import_data}
cafe <- read.table('http://dsr-rennes.github.io/data/cafe_DM.csv', sep = ';' , stringsAsFactors = TRUE, header = TRUE)
```

* Utilisez la commande `summary` pour obtenir un descriptif des données
...
La commande `geom_boxplot`  n'a pas le comportement attendue car `R` traite la variable `Localisation`, un entier compris entre 1 et 6 comme une varaible quantitative. 
...
* Construire maintenant les boîtes à moustache.

#### Calcul de statistiques descriptives


La commande suivante permet de calculer la moyenne de matière sèche pour origine de café.
...
Le table ci-dessous calcule la moyenne, la variance et certains quantiles pour chaque origine de café
...
* Donnez l'expression du modèle statistique M$_1$ permettant de répondre à la question 'Les taux de matière sèche diffèrent ils d'une origine à l'autre ?  Quels sont les paramètres de ce modèle ?

* Quelles sont les hypothèses H$_{0}$ et H$_{1}$ du test de l'existence de différences moyennes entre les lieux de production ? Exprimez ces hypothèses à partir des paramètres du modèle de la question précédente.
...
Enfin, les paramètres estimé dans le modèle complet sont obtenus graàce à la fonction `summary`.
...
*   Donnez l'expression du modèle nul (associé à l'absence de différences moyennes entre les lieux de production comme dans l'hypothèse H$_{0}$ ) des taux de matière sèche. Quelle est la valeur estimée des paramètres de ce modèle ?
...
* En déduire la valeur du coefficient R$^{2}$ du modèle M_$_1$.
...
* Donnez la probabilité qu'une variable aléatoire de loi $\mathcal{F}_{0}$  dépasse la valeur observée sur les données de café et conclure sur les différences de matière sèche en fonction de l'origine du café
